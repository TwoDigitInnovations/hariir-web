...
function BeforeLogin() {
    ...
    const handleSearch = () => {
        ...
        router.push(`/dashboard?country=${selectedCountry}&q=${encodeURIComponent(searchTerm)}`);
    };
                        ...
                        <div className="flex flex-row gap-4 md:w-auto w-full">
                            <div className="relative w-full md:w-[180px] bg-white/50 rounded-[12px]">
                                <select
                                    ...
                                    className="appearance-none text-black/70 px-4 py-2 pr-10 w-full focus:outline-none rounded-[12px]"
                                >
                ...
                <div className="py-4 min-h-[250px]">
                    <div className=" h-[200px] px-4 grid grid-cols-2 md:grid-cols-4 flex-wrap justify-evenly items-center gap-6 md:gap-20 text-center">
                        {stats.map((stat, index) => (
                            <div
                                key={index}
                                className={`py-6 rounded-3xl  text-[#1E1E1E] shadow-[1.5px_2px_2px_0px]  cursor-pointer transform transition-transform duration-300 hover:scale-105`}
                                style={{
                                    background: `linear-gradient(0deg, rgba(255, 255, 255, 0.3), rgba(255, 255, 255, 0.3)), linear-gradient(0deg, rgba(237, 230, 246, 0.2), rgba(237, 230, 246, 0.2))`,
                                }}
                            >
